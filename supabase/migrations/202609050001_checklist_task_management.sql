...
begin
  ...
  from public.checklists
  where id = task_row.checklist_id
  for update;
...
revoke delete on public.checklists, public.tasks from authenticated;

revoke all on function public.create_checklist_task(uuid, text) from public;
revoke all on function public.reorder_checklist_tasks(uuid, uuid[]) from public;
revoke all on function public.soft_delete_checklist(uuid) from public;
