...
4. Reorder робиться кнопками «вгору» / «вниз» без нових залежностей. Drag-and-drop не входить у MVP, бо кнопки одразу дають keyboard і touch доступність. Запис порядку йде через RPC, що перенумеровує всі активні задачі чекліста в одній транзакції. **Це рішення замінено Епіком 3b (DnD через `@dnd-kit`).**
