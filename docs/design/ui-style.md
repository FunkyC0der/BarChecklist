...
- Історія задач → статичний daisyUI `span.status.status-md[role="img"]` біля кожної задачі: виконано — `status-primary`; не виконано — `status-neutral`. Це не кнопки й не чекбокси; виконані рядки лишаються контрастними, а невиконані мають приглушений title. Список невиконаних має видимий overline «Не виконано»; список виконаних пов’язується із заголовком дня.
...
- Sheet → `dialog.modal.items-end.justify-items-center` з shared dock-clearance та `px-3` > `modal-box w-full max-w-xl rounded-box border border-base-300/60 bg-base-100 shadow-xl overflow-y-auto`: floating card завжди має видимі краї, горизонтально центрований і без клавіатури стоїть одразу над dock; з клавіатурою frame слідує `visualViewport` і лишає 12px gap. Heading `text-2xl font-bold tracking-tight`, опційний `IconButton more-horizontal` праворуч і контент. `form.modal-backdrop[method=dialog]` та native Escape закривають dialog; focus trap/restore забезпечує native `dialog`. Використовується для create/edit/confirm/invite.
